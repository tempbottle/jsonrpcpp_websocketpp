...
websocket_sources = ['examples/websocket-server.cpp'];
...
websocketserver = env.Program(target = 'examples/websocket-server', source = [websocket_sources, examples_common], LIBS = libs);
...
env.Alias('examples', ['build', tcpserver, udpserver, tcpclient, udpclient, websocketserver, system_bin]);
